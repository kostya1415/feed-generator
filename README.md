...
1. Реализовать интерфейсы `App\Repository\Contract\OffersRepoInterface` и `App\Repository\Contract\FileRepoInterface`. (`FileRepoInterface` для
   Minio уже реализован в `App\Repository\MinioFileRepo`)
...
1. Скачиваем потоком фид из удалённого в локальное файловое хранилище приложения (потому что сжатие не работает внутри S3).
2. Сжимаем его в zip формат
3. Удаляем не сжатый фид из локальных файлов приложения
4. Загружаем сжатый фид потоком в удалённое хранилище как временный файл
5. Удаляем сжатый фид из локальных файлов приложения
6. Подменяем старый публичный сжатый фид в удалённом хранилище на только что загруженный временный фид
7. Удаляем временный фид в удалённом хранилище
...
1. Скачиваем потоком фид из удалённого в локальное файловое хранилище приложения (потому что сжатие не работает внутри S3). Причём
   скачиваем сразу в gzip.
2. Загружаем сжатый фид потоком обратно в удалённое хранилище как временный файл
3. Удаляем сжатый фид из локальных файлов приложения
4. Подменяем старый сжатый фид в удалённом хранилище на только что загруженный временный файл
5. Удаляем временный файл в удалённом хранилище
